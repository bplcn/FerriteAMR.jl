...
function Ferrite.add!(ch::ConstraintHandler{<:DofHandler{<:Any,<:NonConformingGrid}}, cc::ConformityConstraint, vertexdicts::Vector)
    ...
    for sdh in ch.dh.subdofhandlers
        ...
        field_idx !== nothing && _add_conformity_constraint(ch, field_idx, sdh.field_interpolations[field_idx], vertexdicts)
    end
end

function _add_conformity_constraint(ch::ConstraintHandler, field_index::Int, interpolation::ScalarInterpolation, vertexdicts::Vector)
    for (hdof,mdof) in ch.dh.grid.conformity_info
        ...
        lc = AffineConstraint(vertexdicts[field_index][hdof],[vertexdicts[field_index][m] => 0.5 for m in mdof], 0.0)
        add!(ch,lc)
    end
end

function _add_conformity_constraint(ch::ConstraintHandler, field_index::Int, interpolation::VectorizedInterpolation{vdim}, vertexdicts::Vector) where vdim
    for (hdof,mdof) in ch.dh.grid.conformity_info
        @debug @assert length(mdof) == 2
        ...
        for vd in 1:vdim
            ...
            lc = AffineConstraint(vertexdicts[field_index][hdof]+vd-1,[vertexdicts[field_index][m]+vd-1 => 0.5 for m in mdof], 0.0)
            add!(ch,lc)
        end
    end
end
