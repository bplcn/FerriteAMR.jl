...
function setup_poisson_problem(grid, interpolation, interpolation_geo, qr, N)
    ...
    add!(ch, dbc);
    add!(ch, ConformityConstraint(:u), vertexdicts)
    close!(ch);
...
@testset "analysis" begin
    @testset "$interpolation" for interpolation in (
        ...
        pvd = paraview_collection("example/AMR_test")

        i = 0
        while L2norm > 1e-3 && i < 8
            grid_transfered = FerriteAMR.creategrid(adaptive_grid)
            dh, ch, cellvalues = setup_poisson_problem(grid_transfered, interpolation, interpolation_geo, qr, N)
            ...
            L2norm, _ = check_and_compute_convergence(dh, u, cellvalues, 1e-2, adaptive_grid)
            VTKGridFile("example/p4est_test$(i).vtu", dh) do vtk
